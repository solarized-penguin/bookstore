...
from fastapi import APIRouter, Depends, Body, Query, HTTPException, status, Path
from fastapi.responses import ORJSONResponse
from sqlalchemy import select
from sqlalchemy.ext.asyncio import AsyncSession

from db.models import Car, User, UserPrivileges
from db.session import get_session
from security.auth import UserAuthManager
from .models import CarFilter, build_car_filter_statement, CarCreate, CarUpdate

car_router = APIRouter(prefix="/car", tags=["cars"])


@car_router.get("/", response_model=List[Car], response_class=ORJSONResponse)
async def get_cars(
...
@car_router.get("/{car_id}", response_model=Car, response_class=ORJSONResponse)
async def get_car(
...
@car_router.get("/by_ids/", response_model=List[Car], response_class=ORJSONResponse)
...
@car_router.post("/filter/", response_class=ORJSONResponse, response_model=List[Car])
...
@car_router.post("/add/", response_class=ORJSONResponse, response_model=Car)
...
@car_router.delete("/remove/{car_id}", response_class=ORJSONResponse, response_model=Car)
...
@car_router.patch("/update/{car_id}", response_class=ORJSONResponse, response_model=Car)
